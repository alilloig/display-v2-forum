...
// (resolved template output from the shared Display). Showing them side-by-side makes it
// viscerally clear that the object never changes — only the Display template changes, and
// the rendered output updates.

interface MyHeroesProps {
  // Increment to trigger a refetch (e.g. after a successful mint).
...
export function MyHeroes({ refetchTrigger }: MyHeroesProps) {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['my-heroes', owner, structType, refetchTrigger],
    queryFn: async () => {
      if (!owner || !structType) return [];
  ...
  const label = deployment.schema.typeName;

  if (!owner) {
    return (
      <section style={{ padding: '1rem', border: '1px solid #ddd', borderRadius: 8 }}>
        <h2>My {label}s</h2>
        <p style={{ color: '#999' }}>Connect your wallet to see your {label}s.</p>
      </section>
    );
  }
  ...
  return (
    <section style={{ padding: '1rem', border: '1px solid #ddd', borderRadius: 8 }}>
      <h2 style={{ marginTop: 0 }}>My {label}s</h2>

      {isLoading && <p>Loading…</p>}
      {error && <p style={{ color: 'red' }}>Error: {String(error)}</p>}
      {data && data.length === 0 && <p style={{ color: '#999' }}>No {label}s minted yet.</p>}
      ...
      {data && data.map((item, idx) => {
        ...
        // The RPC sets `error` (and leaves data null) when a template references a field
        // that cannot be resolved — surface it so bad templates are visible.
        const displayError = display?.['error'] as string | null | undefined;

        const objectId = (objData['objectId'] ?? objData['id']) as string | undefined;

        return (
          <div key={objectId ?? idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem', padding: '0.75rem', background: '#f9f9f9', borderRadius: 6 }}>
            <div>
                ...
                <tbody>
                  {Object.entries(fields).map(([k, v]) => (
                    <tr key={k}>
                      ...
                      <td style={{ padding: '2px 0', wordBreak: 'break-all' }}>{typeof v === 'object' ? JSON.stringify(v) : String(v)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div>
              ...
              {displayData && Object.keys(displayData).length > 0 ? (
                ...
                <p style={{ color: '#bbb', fontSize: '0.85rem' }}>No display fields set yet. Use the Display editor to add templates.</p>
              )}
              {displayError && <p style={{ color: 'red', fontSize: '0.8rem', marginTop: 4 }}>Display error: {displayError}</p>}
            </div>
          </div>
        );
      })}
      ...
      {data && data.length > 0 && (
        <button onClick={() => void refetch()} style={{ fontSize: '0.85rem' }}>Refresh</button>
      )}
    </section>
  );
}
