...
// for hands-free localnet testing (a hosted web wallet can't sign localnet txns).
  ...
  const { data: balance, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await (client as unknown as {
        getBalance: (a: { owner: string }) => Promise<{ totalBalance: string }>;
      }).getBalance({ owner: devAddress });
