...
export function App() {
  ...
  async function onMint() {
    setBusy(true);
    try { await run(buildMintHeroTx, 'mintHero'); } finally { setBusy(false); }
  }
    ...
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '1.5rem 1.25rem 4rem', fontFamily: 'system-ui, sans-serif', color: '#111' }}>
      ...
      {!deployed && (
        <div style={{ padding: 12, background: '#fef2f2', color: '#991b1b', borderRadius: 8, fontSize: '0.85rem' }}>
          No deployment found. Run <code>pnpm publish:localnet</code> (from <code>hero-frontend/</code>) to publish the package and generate <code>deployment.ts</code>.
        </div>
      )}
      ...
      <main style={{ marginTop: 16 }}>
        {!address ? (
          <div style={{ padding: '2rem', textAlign: 'center', border: '1px dashed #d1d5db', borderRadius: 12, color: '#6b7280' }}>
            Connect a wallet (or switch to the <strong>Dev key</strong> above for hands-free localnet signing) to begin.
          </div>
        ) : isLoading ? (
